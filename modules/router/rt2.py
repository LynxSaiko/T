...
class NetworkScanner:
    ...
    ROUTER_BRANDS = {
        ...
        "Huawei": {
            ...
            "paths": ["/", "/login.asp", "/userRpm/StatusRpm.htm"],
        ...
        "First Media": {
            ...
            "http_keywords": [r"first media", r"docsis cable modem", r"arris", r"hitron", r"firstmedia", r"cable modem"],
            "ports": [80, 443, 8080],
            "paths": ["/", "/login", "/cmconfig"],
            ...
            try:
                ...
                for pat in self.FIRMWARE_PATTERNS:
                    ...
                    if m:
                        ...
                        break
                return {"type": "L4", "banner": banner[:400], "firmware": firmware}
            except:
            ...
            for path in paths:
                ...
                try:
                    ...
                    if not http_data['favicon_hash']:
                        try:
                            fav_resp = requests.get(f"{scheme}://{ip}:{port}/favicon.ico", timeout=1, verify=False, headers=headers)
    ...
    def detect_router_brand(self, open_ports):
        ...
        if brand_scores:
            best_brand, best_score = max(brand_scores.items(), key=lambda x: x[1])
            return best_brand
                ...
                try:
                    ...
                    if fingerprint:
                        open_ports.append({
                            ...
                            'firmware': (fingerprint.get('firmware') or "") if isinstance(fingerprint, dict) else ""
                        })
                    else:
                        open_ports.append({
                            'port': port,
            ...
            with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
                ...
                for future in as_completed(future_to_ip):
                    ...
                    with lock:
                        ...
                        pbar.set_postfix({"hosts_found": self.scan_stats['active_hosts'], "routers": self.scan_stats['routers_found']})
                        pbar.update(1)
    ...
    def display_results(self):
        if not self.discovered_hosts:
            self.console.print(Panel("[yellow]ⓘ No active hosts discovered[/yellow]", border_style="yellow", box=box.ROUNDED))
            return

        self.discovered_hosts.sort(key=lambda x: (x['is_router'], len(x['open_ports'])), reverse=True)

        self.console.print(Panel("[bold cyan]🌐 NETWORK DISCOVERY RESULTS[/bold cyan]", border_style="cyan", box=box.DOUBLE))
        ...
        for i, host in enumerate(self.discovered_hosts, 1):
            ...
            if host['open_ports']:
                ports_list = []
                for p in host['open_ports'][:3]:
                    fw = p.get('firmware') or ""
                    entry = f"{p['port']}/{p['service']}{_short_fw(fw)}"
            ...
            hosts_table.add_row(str(i), host['ip'], host['hostname'] or "[dim]-[/dim]", status, display_brand, ports_text)

        self.console.print(hosts_table)
        self.console.print("")

        router_count = sum(1 for host in self.discovered_hosts if host['is_router'])
        router_brands = set(h['router_brand'] for h in self.discovered_hosts if h['is_router'] and h['router_brand'] and h['router_brand'] != "Generic Web Device")
        ...
        self.console.print(Panel(summary_table, title="[bold green][*] SCAN COMPLETE [*][/bold green]", border_style="green", box=box.DOUBLE))
...
def run(session, options):
    ...
    console.print(Panel(config_table, title="[bold blue][*] SCAN CONFIGURATION[/bold blue]", border_style="blue", box=box.ROUNDED))
    console.print("")

    scanner = NetworkScanner(
        console=console, target=target, ports=ports_config, timeout=timeout,
        max_threads=max_threads, ping_check=ping_check
    )
    ...
    try:
        ...
        console.print(Panel("[yellow]⚠ Scan interrupted by user[/yellow]", border_style="yellow", box=box.ROUNDED))
    except Exception as e:
